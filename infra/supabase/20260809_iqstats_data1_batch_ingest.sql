...
do $$
begin
  if exists (select 1 from football.competitions)
    or exists (select 1 from football.seasons)
    or exists (select 1 from football.teams)
    or exists (select 1 from football.venues)
    or exists (select 1 from football.referees)
    or exists (select 1 from football.matches)
    or exists (select 1 from football.standing_snapshots)
  then
    raise exception 'DATA-1 batch migration requires empty football tables'
      using errcode = '55000';
  end if;
end;
$$;
...
alter table football.seasons
  add column content_checksum text not null;
alter table football.teams
  add column content_checksum text not null;
alter table football.venues
  add column content_checksum text not null;
alter table football.referees
  add column content_checksum text not null;

alter table football.competitions
  add constraint competitions_checksum_not_blank check (btrim(content_checksum) <> '');
alter table football.seasons
  add constraint seasons_checksum_not_blank check (btrim(content_checksum) <> '');
alter table football.teams
  add constraint teams_checksum_not_blank check (btrim(content_checksum) <> '');
alter table football.venues
  add constraint venues_checksum_not_blank check (btrim(content_checksum) <> '');
alter table football.referees
  add constraint referees_checksum_not_blank check (btrim(content_checksum) <> '');
